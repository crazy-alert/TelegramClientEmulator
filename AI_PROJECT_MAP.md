...
- `src/Application.php` — composition root, custom router, orchestration dashboard/media/local routes; локальные Bot API requests делегируются в `BotApiController`, chat-сценарии — в `ChatController`, inspector/update/delivery UI routes — в `InspectorController`, admin UI ботов и пользователей — в `BotAdminController` и `ProfileAdminController`, import/export — в `ImportExportController`.
