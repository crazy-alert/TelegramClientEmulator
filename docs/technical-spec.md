...
- Пользователь может открыть `/group-chats`, увидеть список group/supergroup чатов, редактировать title, управлять участниками через существующие profiles и задавать базовую роль `member` или `administrator`.
- Изменение title, добавление участника и удаление участника через group UI создают локальные service messages в истории группы без Bot API update.
...
- Дальнейшее развитие group chat: более подробные Telegram permissions и дополнительные service events поверх уже существующих `chats/chat_members`.
