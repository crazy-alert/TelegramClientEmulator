...
- Webhook delivery уже вынесена из `Application` в `WebhookDeliveryService`; дальнейшая декомпозиция должна аналогично выносить Bot API и Chat UI handlers без изменения HTTP-контрактов.
