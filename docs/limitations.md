...
- `timeout` ограничен коротким ожиданием до `LONG_POLLING_MAX_TIMEOUT_SECONDS` секунд, по умолчанию `3`, чтобы не блокировать single-process встроенный PHP server надолго;
